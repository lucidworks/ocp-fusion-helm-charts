...
{{- if and (include "kafka.tlsEncryption" .) (eq .Values.auth.tls.type "jks") (empty .Values.auth.tls.existingSecrets)  (not ( eq (include "fusion.tls.enabled" .) "true" ))  }}
...
{{- else if and (include "kafka.tlsEncryption" .) (eq .Values.auth.tls.type "pem") (empty .Values.auth.tls.existingSecrets) (not .Values.auth.tls.autoGenerated) (not ( eq (include "fusion.tls.enabled" .) "true" )) }}
kafka: auth.tls.existingSecrets
