...
{{- else -}}
{{- range $i := until ( int ( $zkReplicas )) -}}
{{- if ne $i 0 }},{{ end }}{{- printf "%s-zookeeper-%d.%s-%s:%d" $.Release.Name $i $.Release.Name "zookeeper-headless" (ternary (int $zkTLSPort) (int $zkPort) $tlsEnabled) -}}{{ if $zkSolrChrootPath }}{{ $zkSolrChrootPath }}{{ end -}}
{{- end -}}
{{- end -}}
